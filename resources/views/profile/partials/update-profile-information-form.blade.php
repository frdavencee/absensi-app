    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div class="flex items-center gap-4 mb-4">
            <div class="w-20 h-20 rounded-full overflow-hidden bg-gray-200 border border-gray-300">
                @if($user->foto)
                    <img src="{{ asset('storage/foto/' . $user->foto) }}" alt="Foto Profile" class="w-full h-full object-cover">
                @else
            ...
            <div>
                ...
                <input type="file" id="foto" name="foto" accept="image/*" class="border border-gray-300 rounded-lg px-3 py-2 text-sm">
            </div>
        </div>
